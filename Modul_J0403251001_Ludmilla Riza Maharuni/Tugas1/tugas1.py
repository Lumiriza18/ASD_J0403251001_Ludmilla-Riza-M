...
def baca_data(file_name):
    ...
    with open(file_name, "r", encoding="utf-8") as file: #membuaka file dalam mode baca(read).with memastikan file tertutup otomatis
        for baris in file:
            baris = baris.strip()
            if not baris:
                continue
...
def simpan_data(file_name, data_dict):
